...
with _:
    ...
    f
    # val: Scratch(a=['A', 'B', 'D', 'E', 'F'], find=<function __sc_14__.<locals>.__sc_13__.<locals>.find at 0x7b7b8e18f4c0>, out=['Searching range from index 0 to 4', 'Searching range from index 0 to 2', 'Searching range from index 2 to 2', 'C is not in the list'], err=[], outs='Searching range from index 0 to 4\nSearching range from index 0 to 2\nSearching range from index 2 to 2\nC is not in the list')
####################################################
...
with _:
    ...
    with "main.py" as f:
        ...
        # !err:   at line 934: number = int(input())
    f.out
...
Base case 2: If range_size is equal to 1, then output the value of match_char, followed by " is not in the list".
Recursive case:
If match_char < mid_char is True, then search for match_char between lower_index and mid_index.
Otherwise, search for match_char between mid_index + 1 and upper_index.
Click here for descriptions of variables in search()
char_list is the list of characters being searched.
match_char is the character being searched for.
lower_index and upper_index are the current lower and upper bounds of the search range.
mid_index is the middle index between lower_index and upper_index.
range_size is the number of elements in the search range.
mid_char is the element at index mid_index of char_list.
