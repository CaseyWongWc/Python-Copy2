...
# v7 auto-show annotations look like `# <name>.out: ...`. Strip them on
...
# `with RUN:` / `with Sandbox:` and the same with `<args>` tail.
# `Sandbox` is a v7 alias of `RUN` — same behavior, clearer name. Old
# `RUN` blocks keep working untouched.
WITH_RUN_RE = re.compile(r'^(\s*)with\s+(?:RUN|Sandbox)\s*:\s*(.*?)\s*$')
# v7 short save+run+capture shape used by the tokenize-failure
# fallback path: `with "FILENAME.py" as <name>:` where <name> is any
# identifier OTHER than the reserved scratch/RUN/Sandbox names. The
# token-walk path above does the same routing token-by-token; this
# regex only kicks in when tokenize() raised on the source.
WITH_SAVE_CAPTURE_RE = re.compile(
    r'^(\s*)with\s+(["\'])(.+?)\2\s+as\s+(\w+)\s*:\s*$'
)
...
def _find_magic_with_lines(src: str):
    ...
    try:
        ...
        for ln_idx, line in enumerate(src.splitlines(), start=1):
            ...
            if m_ssr:
                ...
                save_run_info[ln_idx] = (path, capture)
                continue
            m_sc = WITH_SAVE_CAPTURE_RE.match(stripped_line)
            if m_sc and m_sc.group(4) not in (
                    "Scratch", "_", "__", "RUN", "Sandbox"):
                # NEW v7 short save+run+capture shape:
                # `with "FILENAME.py" as <name>:` (name is any
                # identifier other than reserved scratch/RUN names).
                path = m_sc.group(3)
                capture = m_sc.group(4)
                save_run_info[ln_idx] = (path, capture)
                continue
    ...
    for i, t in enumerate(code):
        ...
        if a.type == tokenize.STRING and i + 4 < len(code):
            ...
            # NEW v7 short shape: `with "FILENAME.py" as <name>:` where
            # <name> is anything OTHER than the reserved scratch/RUN
            # keywords. Treat as save+run+capture: write the body to
            # disk, run it in-process under a Scratch namespace, bind
            # the result to <name>. Auto-show `# <name>.out:` lines
            # land below the block (handled by the shim's __CAP__
            # markers + the splice loop).
            #
            # `with "X" as Scratch:` / `as RUN:` / `as Sandbox:` /
            # `as _:` / `as __:` are intentionally excluded so the
            # existing reserved-name routes still win.
            _RESERVED = {"Scratch", "_", "__", "RUN", "Sandbox"}
            if (is_as_form
                    and scratch_tok.type == tokenize.NAME
                    and scratch_tok.string not in _RESERVED
                    and after_scratch.type == tokenize.OP
                    and after_scratch.string == ":"):
                try:
                    path_str = ast.literal_eval(a.string)
                except Exception:
        ...
        if a.type == tokenize.STRING and i + 4 < len(code):
            ...
            run_match = (
                ...
                and run_tok.string in ("RUN", "Sandbox")
        ...
        if a.type == tokenize.NAME and a.string in ("RUN", "Sandbox"):
            if b.type == tokenize.OP and b.string == ":":
...
def _build_shim(read_path: Path, file_attr_path: Path,
    ...
    return (
        ...
        "\n"
        # v7: __nb_capture__ now also takes the binding name and the
        # block's end-line so we can emit `__CAP__` markers (auto-show
        # `# <name>.out:` lines under the block) AND `__ERR__` markers
        # (per-block crash isolation: the exception is reported next
        # to the block, the rest of the notebook keeps going).
        "import traceback as _tb\n"
        "def __nb_capture__(ns_cls, fn, name=None, end_line=0):\n"
        "    cap = {'out': [], 'err': []}\n"
        ...
        "        _capture_stack.pop()\n"
        # Auto-show: one `__CAP__` marker per captured stdout line. The
        # splice loop turns these into `# <name>.out: <line>` annotations
        # right under the block's last body line. Skip when no name was
        # bound (plain `with Scratch:` users opt out automatically).\n
        "    if name and end_line:\n"
        "        for piece in cap['out']:\n"
        "            sys.stdout.write(f'__CAP__{end_line}|||{name}|||{piece}\\n')\n"
        "        sys.stdout.flush()\n"
        # Crash isolation: emit `__ERR__` markers per block frame in
        # the user's notebook file (filtered to _NB_FILE so internal
        # `__sc_N__` wrapper frames don't pollute the report). The
        # notebook continues — exceptions never bubble out of the
        # block. Type + message lands first, then the user-facing
        # frames as `at goog.py:<lineno>` lines.\n
        "    if crashed is not None and end_line:\n"
        "        msg = f'{type(crashed).__name__}: {crashed}'\n"
        "        sys.stdout.write(f'__ERR__{end_line}__{msg}\\n')\n"
        "        for fr in _tb.extract_tb(crashed.__traceback__):\n"
        "            if fr.filename == _NB_FILE:\n"
        "                sys.stdout.write(\n"
        "                    f'__ERR__{end_line}__  at line {fr.lineno}: {fr.line or \"\"}\\n'\n"
        "                )\n"
        "        sys.stdout.flush()\n"
        "    ns = ns_cls(result if result is not None else {})\n"
        ...
        "                        args=[], keywords=[])\n"
        # v7: every Scratch block (capture or not) routes through
        # __nb_capture__ so we get crash isolation + (when bound)
        # auto-show `# <name>.out:` markers. The end-line is taken
        # from the last body statement so annotations land at the
        # bottom of the block, where Casey expects to read them.\n
        "        last_body = node.body[-1] if node.body else node\n"
        "        end_line = getattr(last_body, 'end_lineno', None) or last_body.lineno\n"
        "        if item.optional_vars is not None:\n"
        "            name_str = getattr(item.optional_vars, 'id', None) or ''\n"
        "            wrapped = ast.Call(\n"
        "                func=ast.Name(id='__nb_capture__', ctx=ast.Load()),\n"
        "                args=[ast.Name(id='__NB_NS__', ctx=ast.Load()),\n"
        "                      ast.Name(id=fname, ctx=ast.Load()),\n"
        "                      ast.Constant(value=name_str),\n"
        "                      ast.Constant(value=end_line)],\n"
        "                keywords=[])\n"
        ...
        "            return [func, assign]\n"
        "        else:\n"
        # No-capture form: still go through __nb_capture__ so a crash
        # inside the body becomes a `# !err:` annotation under the
        # block instead of taking down the whole notebook. name='' so
        # auto-show stays off.
        "            wrapped = ast.Call(\n"
        "                func=ast.Name(id='__nb_capture__', ctx=ast.Load()),\n"
        "                args=[ast.Name(id='__NB_NS__', ctx=ast.Load()),\n"
...
def _run_shim(shim_src: str):
    ...
    out_map, val_map = {}, {}
    # v7 maps:
    #   cap_map[lineno] = [(name, piece), ...]   for `# <name>.out:` auto-show
    #   block_err_map[lineno] = [text, ...]      for per-block `# !err:` lines
    cap_map: dict = {}
    block_err_map: dict = {}
    for line in proc.stdout.splitlines():
        ...
        if m:
            ...
            continue
        m = re.match(r"__CAP__(\d+)\|\|\|([^|]+)\|\|\|(.*)", line)
        if m:
            cap_map.setdefault(int(m.group(1)), []).append(
                (m.group(2), m.group(3))
            )
            continue
        m = re.match(r"__ERR__(\d+)__(.*)", line)
        if m:
            block_err_map.setdefault(int(m.group(1)), []).append(m.group(2))
            continue
    err_lines = []
    if proc.returncode != 0 and proc.stderr.strip():
        err_lines = ["--- ERROR ---"] + proc.stderr.splitlines()
    return out_map, val_map, cap_map, block_err_map, err_lines
...
def run_and_annotate(path: str, *, skip_none: bool = False,
    ...
    if preflight_err_lines is not None:
        ...
        out_map, val_map, cap_map, block_err_map, err_lines = (
            {}, {}, {}, {}, preflight_err_lines
        )
    else:
        ...
        try:
            ...
            out_map, val_map, cap_map, block_err_map, err_lines = _run_shim(
                shim
            )
        finally:
            try:
    ...
    for idx, line in enumerate(cleaned.splitlines(), start=1):
        ...
        # v7: auto-show `# <name>.out: <piece>` lines under any
        # block bound `as <name>`. Comes from the shim's __CAP__
        # markers via _run_shim.
        if idx in cap_map:
            for cap_name, piece in cap_map[idx]:
                new_lines.append(f"{indent}# {cap_name}.out: {piece}")
        # v7: per-block crash isolation — `__ERR__` markers from the
        # shim become `# !err:` lines next to the failing block while
        # the rest of the notebook keeps running.
        if idx in block_err_map:
            for text in block_err_map[idx]:
                new_lines.append(f"{indent}{ERR_PREFIX} {text}")
