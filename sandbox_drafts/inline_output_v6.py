...
# Cache of builtin names — computed once per process. Includes builtin
# functions/types AND Python keywords so we catch typos like `prnit` →
# `print` and `els` → `else`.
_BUILTIN_NAMES = frozenset(dir(builtins)) | frozenset(keyword.kwlist)


def _collect_defined_names(src: str) -> set:
    """Walk the source's AST and collect every NAME a typo could reasonably
    have meant to reference: top-level assigns, function/class defs,
    imports, for-loop targets, and comprehension targets. We skip names
    inside nested scopes only loosely — `difflib.get_close_matches`
    decides what's actually similar, and false positives in the suggestion
    pool are harmless (we only show CLOSE matches anyway).
    """
    names: set = set()
    try:
        tree = ast.parse(src)
...
_NAME_ERR_RE = re.compile(r"NameError: name '([^']+)' is not defined")
_ATTR_ERR_RE = re.compile(
    r"AttributeError: '([^']+)' object has no attribute '([^']+)'"
)


def _attr_pool_for(type_name: str):
    """Return the dir() of a builtin type if `type_name` matches one we
    can recognize (str, list, dict, set, tuple, int, float, bytes, etc.).
    Used to power `did you mean?` hints on AttributeError. For non-builtin
    types we have no way to enumerate attributes from outside the
    subprocess, so we return None and skip the hint."""
    candidate = getattr(builtins, type_name, None)
    if isinstance(candidate, type):
        return [a for a in dir(candidate) if not a.startswith("_")]
    return None


def _augment_traceback_with_hints(err_lines, source_for_names: str):
    """Scan err_lines for NameError / AttributeError messages and append a
    `  ↳ did you mean: foo, bar?` hint immediately after each match.
    Hints come from:
      - NameError: AST-defined names in the source + builtins + keywords
      - AttributeError: dir() of the offending builtin type, when the type
        is a builtin we can recognize
    Hints are added only when difflib finds at least one close match
    (cutoff=0.6) — silence is better than a noisy false suggestion.
    """
    if not err_lines:
        return err_lines

    name_pool = list(_collect_defined_names(source_for_names) | _BUILTIN_NAMES)
    out = []
    for line in err_lines:
        out.append(line)
        m = _NAME_ERR_RE.search(line)
        if m:
            bad = m.group(1)
            matches = difflib.get_close_matches(bad, name_pool, n=3, cutoff=0.6)
            if matches:
                out.append(f"  ↳ did you mean: {', '.join(matches)}?")
            continue
        m = _ATTR_ERR_RE.search(line)
        if m:
            type_name, bad_attr = m.group(1), m.group(2)
            attr_pool = _attr_pool_for(type_name)
            if attr_pool:
                matches = difflib.get_close_matches(
                    bad_attr, attr_pool, n=3, cutoff=0.6
                )
                if matches:
                    out.append(
                        f"  ↳ did you mean: {', '.join(matches)} (on {type_name})?"
                    )
    return out
...
def run_and_annotate(path: str, *, skip_none: bool = False,
    ...
    if preflight_err_lines is not None:
        ...
        # Append `did you mean: ...?` hints under any NameError /
        # AttributeError lines in the traceback. Pre-flight errors are
        # already friendly (see above) so we only augment the shim's
        # post-run traceback.
        err_lines = _augment_traceback_with_hints(err_lines, cleaned)
